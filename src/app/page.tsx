    ...
    <div className="flex flex-col items-center bg-[var(--background)]">
      ...
      <StoreCarousel title={<>Son çıkanlar. <span className="text-[var(--muted-foreground)]">Yeni olanlara göz atın.</span></>}>
        {loading ? (
          ...
          newProducts.map((p, i) => (
            <Link href={`/urunler/${p.id}`} key={p.id} className="snap-always flex-shrink-0 w-[300px] sm:w-[400px] bg-[var(--card)] border border-[var(--border)] rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow group flex flex-col justify-between">
